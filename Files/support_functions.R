#IMPORTANT: The first function was downloaded from the nflreadr package
#The second function was created by me
...
#Create Season Data Function to extract all necessary Data for our analysis
#This function essentialy cleans the data, removes kickers that have played less than 10 games
#and combines players that have been traded
season_data <- function(data){
  player_name <- data$player_name
  df <- data.frame(index = player_name) 
  df <- df %>% rename(player_name = index)
  #We now have a data frame with the player names
  
  df$pat_att <- data$pat_att
  df$pat_made <- data$pat_made
  df$fg_made <- data$fg_made_list
  df$fg_missed <- data$fg_missed_list
  df$games <- data$games
  
  #If a player has been traded he shows up twice: these two are combined
  df <- df %>%
    group_by(player_name) %>%
    summarize(  pat_att = sum(pat_att),
                pat_made = sum(pat_made),
                fg_made = paste(fg_made, collapse = ";"),
                fg_missed = paste(fg_missed, collapse = ";"),
                games = sum(games))
  
  #Drop any player that has played less than 10 games total                
  df <- df[df$games >= 10, ]
  df <- df %>% select(-games)
...
#Player season function returns a 2 row matrix with the first colum being the PAT information
#The rest has the distance of the Kick in the upper row and a success indicator in the lower row

player_season <- function (season_data, n) {
    data <- season_data[n,]
    v1 <- as.numeric()
    v2 <- as.numeric()
    
    v1 <- as.integer(strsplit(data$fg_made, ";")[[1]])
    v1 <- v1[!is.na(v1)]
    v2 <- as.integer(strsplit(data$fg_missed, ";")[[1]])
    v2 <- v2[!is.na(v2)]

    first_line <- c(as.integer(data$pat_att), v1, v2)
    second_line <- c(as.integer(data$pat_made), rep(1, length(v1)),
                     rep(0, length(v2)))
    df <- rbind(first_line , second_line)

    #This returns a vector where the first colum is the pat information
    #and the rest is the field goal information
    return(df)
}
